# Linux (davincibox) 環境向け セットアップ・起動ガイド

「りぞりぷと」は、DaVinci Resolve での動画制作（立ち絵・字幕・音声作成など）を強力に支援する動画制作補助ツールです。

本ガイドでは、Linux / Bazzite (Universal Blue) 等の環境において、[davincibox][davincibox_link] コンテナ上で「りぞりぷと」および DaVinci Resolve をセットアップ・起動する手順を説明します。
...
1. [davincibox][davincibox_link] をセットアップします。
   ...
   - スクリプトを実行してコンテナを作ります。
      ...
      `Add DaVinci Resolve launcher? Y/n`
      と聞かれますが、ここでは `n` でスキップ（スキップを選択）してください。

      > [!WARNING]
      > **【重要・注意】**
      > `n` を選択した場合でも、`davincibox` のセットアップ仕様により **デスクトップ上に DaVinci Resolve のランチャー (`.desktop` ファイル) が自動配置** されてしまいます。
      > デスクトップ上のランチャーから DaVinci Resolve を直接起動した場合、必要な環境変数や連携設定が適用されないため **「りぞりぷと」は正常に機能しません** 。
      > デスクトップに配置されたランチャーは使用せず削除するか無視していただき、必ず後述の `./setup_launcher.sh` で登録するホストのアプリケーションランチャー（または `Resolve.sh` スクリプト）から起動してください。
...
ホストのアプリケーションランチャーの `りぞりぷと` からも起動できます。
...
ホストのアプリケーションランチャーの `DaVinci Resolve` から起動できます。

> [!CAUTION]
> **※ デスクトップ上に配置されている DaVinci Resolve アイコンから起動しないでください。**
> デスクトップ上のアイコンは davincibox インストール時に自動作成されたものであり、「りぞりぷと」に必要な環境変数が読み込まれず動作不具合の原因となります。必ずホストのアプリケーションランチャーまたは `./Resolve.sh` から起動してください。

---

## 環境変数のカスタマイズ（AMD / Intel GPU 向け）

AMD または Intel GPU をご使用の環境で DaVinci Resolve 起動時に OpenCL や GPU が認識されない場合、Mesa の **RustiCL (OpenCL 3.0)** を有効化することで動作する場合があります。

`data/app/env.json` の `rusticl` セクションの `vars` ブロックを以下のように編集してください。

```json
"rusticl": {
  "description": "AMD / Intel GPU 用 RustiCL (OpenCL 3.0) 有効化設定",
  "vars": {
...
※ NVIDIA GPU ユーザーや、AMD で ROCm ドライバを別途セットアップ済みの方は編集不要（デフォルトの `vars: {}` のまま）です。
